...
- Формат: JSON-массив организаций, структура полностью совпадает с `OrganizationDto` (можно вложить coordinates/postalAddress/officialAddress/town в той же записи). Шаблон доступен по эндпоинту `GET /api/imports/template` и в файле `docs/import-sample.json`.
- Загрузка: `POST /api/imports` (multipart form-data, поле `file`). Идентификатор пользователя можно передать заголовком `X-User` или параметром `username`, флаг администратора — `admin=true` (для просмотра чужой истории).
- История: `GET /api/imports` — операции пользователя, админ видит все; `GET /api/imports/{id}` — подробности. Фиксируются id, статус, запускавший пользователь, добавленное количество (только при успехе), ошибка и имя файла.
- Транзакционность: импорт обрабатывается одной транзакцией уровня `SERIALIZABLE`; при любой ошибке весь пакет откатывается, в историю пишется статус `FAILED` с текстом ошибки.

## Прикладные ограничения уникальности

Ограничения проверяются в бизнес-логике (не в БД) при создании/обновлении/импорте:
- `fullName` организации — уникально (без учета регистра).
- Пара координат `(x, y)` организации — уникальна.
- Почтовый индекс `postalAddress.zipCode` — уникален среди организаций.

При нарушении возвращается 400 с текстом ограничения.

## JMeter сценарий и изоляция транзакций

- План JMeter: `docs/jmeter-plan.jmx` (сервер/порт и целевые id заданы переменными `host`, `port`, `targetId`, `deleteId`). Сценарий одновременно выполняет обновление `targetId`, удаление `deleteId`, массовое создание, проверку уникальности (намеренный конфликт `fullName`), а также импорт файла `docs/import-sample.json`.
- Массовые операции и модификации работают на уровне изоляции `SERIALIZABLE`, чтобы избежать гонок при проверке прикладных уникальностей (результаты JMeter при параллельных запросах без повышения изоляции давали дубли `fullName`/координат). Чтения остаются `READ COMMITTED`.
